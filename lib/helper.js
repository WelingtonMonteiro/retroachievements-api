class Helper {
  static gameList() {
    return [
      {ID: '1', Name: 'Mega Drive'},
      {ID: '2', Name: 'Nintendo 64'},
      {ID: '3', Name: 'SNES'},
      {ID: '4', Name: 'Game Boy'},
      {ID: '5', Name: 'Game Boy Advance'},
      {ID: '6', Name: 'Game Boy Color'},
      {ID: '7', Name: 'NES'},
      {ID: '8', Name: 'PC Engine'},
      {ID: '9', Name: 'Sega CD'},
      {ID: '10', Name: '32X'},
      {ID: '11', Name: 'Master System'},
      {ID: '12', Name: 'PlayStation'},
      {ID: '13', Name: 'Atari Lynx'},
      {ID: '14', Name: 'Neo Geo Pocket'},
      {ID: '15', Name: 'Game Gear'},
      {ID: '16', Name: 'GameCube'},
      {ID: '17', Name: 'Atari Jaguar'},
      {ID: '18', Name: 'Nintendo DS'},
      {ID: '19', Name: 'Wii'},
      {ID: '20', Name: 'Wii U'},
      {ID: '21', Name: 'PlayStation 2'},
      {ID: '22', Name: 'Xbox'},
      {ID: '23', Name: 'Magnavox Odyssey 2'},
      {ID: '24', Name: 'Pokemon Mini'},
      {ID: '25', Name: 'Atari 2600'},
      {ID: '26', Name: 'DOS'},
      {ID: '27', Name: 'Arcade'},
      {ID: '28', Name: 'Virtual Boy'},
      {ID: '29', Name: 'MSX'},
      {ID: '30', Name: 'Commodore 64'},
      {ID: '31', Name: 'ZX81'},
      {ID: '32', Name: 'Oric'},
      {ID: '33', Name: 'SG-1000'},
      {ID: '34', Name: 'VIC-20'},
      {ID: '35', Name: 'Amiga'},
      {ID: '36', Name: 'Atari ST'},
      {ID: '37', Name: 'Amstrad CPC'},
      {ID: '38', Name: 'Apple II'},
      {ID: '39', Name: 'Saturn'},
      {ID: '40', Name: 'Dreamcast'},
      {ID: '41', Name: 'PlayStation Portable'},
      {ID: '42', Name: 'Philips CD-i'},
      {ID: '43', Name: '3DO Interactive Multiplayer'},
      {ID: '44', Name: 'ColecoVision'},
      {ID: '45', Name: 'Intellivision'},
      {ID: '46', Name: 'Vectrex'},
      {ID: '47', Name: 'PC-8000/8800'},
      {ID: '48', Name: 'PC-9800'},
      {ID: '49', Name: 'PC-FX'},
      {ID: '50', Name: 'Atari 5200'},
      {ID: '51', Name: 'Atari 7800'},
      {ID: '52', Name: 'X68K'},
      {ID: '53', Name: 'WonderSwan'},
      {ID: '54', Name: 'Cassette Vision'},
      {ID: '55', Name: 'Super Cassette Vision'},
      {ID: '56', Name: 'Neo Geo CD'},
      {ID: '57', Name: 'Fairchild Channel F'},
      {ID: '58', Name: 'FM Towns'},
      {ID: '59', Name: 'ZX Spectrum'},
      {ID: '60', Name: 'Game & Watch'},
      {ID: '61', Name: 'Nokia N-Gage'},
      {ID: '62', Name: 'Nintendo 3DS'},
      {ID: '63', Name: 'Watara Supervision'},
      {ID: '64', Name: 'Sharp X1'},
      {ID: '65', Name: 'TIC-80'},
      {ID: '66', Name: 'Thomson TO8'},
      {ID: '100', Name: 'Hubs'},
      {ID: '101', Name: 'Events'}
    ]

  }
  static apiMethodList () {
    return {
      getConsoleIDs: "API_GetConsoleIDs.php",

      getUserRankAndScore: "API_GetUserRankAndScore.php",
      getTopTenUsers: "API_GetTopTenUsers.php",
      getUserRecentlyPlayedGames: "API_GetUserRecentlyPlayedGames.php",
      getUserSummary: "API_GetUserSummary.php",
      getUserCompletedGames: "GetUserCompletedGames.php",
      getUserCompletedGames2: "API_GetUserCompletedGames.php",
      getUserProgress: "API_GetUserProgress.php",
      getFeedFor: "API_GetFeed.php",
      getUserGameRankAndScore: "API_GetUserGameRankAndScore.php",

      getGameList: "API_GetGameList.php",
      getGameInfo: "API_GetGame.php",
      getGameInfoExtended: "API_GetGameExtended.php",
      getGameInfoAndUserProgress: "API_GetGameInfoAndUserProgress.php",
      getGameRankAndScore: "API_GetGameRankAndScore.php",
      getGameRating: "API_GetGameRating.php",

      getAchievementsEarnedOnDay: "API_GetAchievementsEarnedOnDay.php",
      getAchievementsEarnedBetween: "API_GetAchievementsEarnedBetween.php",
      getAchievementCount: "API_GetAchievementCount.php",
      getAchievementUnlocks: "API_GetAchievementUnlocks.php",

      //new
      getTicketData: "API_GetTicketData.php",

    }
  }
}

module.exports = Helper
